...
# alternatively, we can download the dict
# "https://alanlorenzetti.github.io/halo_nr_tx/data/dictionary.tsv"
dict_file = "data/dictionary.tsv"

# used in halo atlas
dict = read_tsv(dict_file) %>% 
  filter(!str_detect(representative, "VNG_t|VNG_r|VNG_s")) %>% 
  tidyr::separate_rows(locus_tag, sep = ",") %>% 
  filter(!str_detect(locus_tag, "_")) %>% 
  filter(!str_detect(locus_tag, "^OE")) %>% 
  rename(locus_tag = "query_id",
         representative = "subject_id") %>% 
  select(query_id, subject_id)

dict_complete = read_tsv(dict_file) %>% 
  filter(!str_detect(representative, "VNG_t|VNG_r|VNG_s")) %>% 
  tidyr::separate_rows(locus_tag, sep = ",")